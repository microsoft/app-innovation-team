            ...
            using (var handler = new HttpClientHandler())
            {
                ...
                using (HttpClient client = new HttpClient(handler))
                {
                    ...
                    using (var content = new ByteArrayContent(byteData))
                    {
                        ...
                        var response = await client.PostAsync($"{Settings.LuisRouterUrl}/appauthentication", content);
                    ...
                    using (HttpClient client = new HttpClient(handler))
                    {
                        try
                        {
                            ...
                            using (var content = new ByteArrayContent(byteData))
                            {
                                ...
                                var response = await client.PostAsync($"{Settings.LuisRouterUrl}/luisdiscovery", content);
