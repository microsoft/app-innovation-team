...
namespace RF.ContentSearch.Api.Controllers
{
    ...
    [ApiController]
    public class ContentSubmissionController : Controller
    {
        private readonly ILogger logger = null;

        public ContentSubmissionController(ILogger<ContentSubmissionController> logger)
        {
            this.logger = logger;
        }
        ...
        public async Task<IActionResult> Post([FromBody]ContentSubmissionRequest model)
        {
            // non-forced-to-disposal
            ContentSubmissionResponse result = new ContentSubmissionResponse
            {
                IsSucceded = true,
            ...
            try
            {
                ...
                {
                    ContentSubmissionMessage contentSubmissionMessage = new ContentSubmissionMessage()
                    {
                        Approver = model.Approver,
                        Bidder = model.Bidder,
                        ConentUri = model.ContentUri
                    };

                    await messageQueueHelper.QueueMessageAsync(contentSubmissionMessage, ApplicationSettings.ContractDeploymentQueueName, keyVaultConnectionInfo);
                }
            }
            catch (Exception ex)
            {
